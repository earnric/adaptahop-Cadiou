...
!*************************************************************************
  subroutine init_liste_halos
    ...
    do hno = 1,max_groups
       ...
       liste_halos(hno)%datas%Reff               = 0.0
       ! shape type
  ...
  subroutine new_step
    ...
    integer(kind=4)        :: i,ierr,good_halos,good_subs
    real(kind=8),parameter :: rel_prec = 1e-5 
    real(kind=4)           :: read_time_ini,read_time_end
    real(kind=8)           :: macc

    write(errunit,*) 
    write(errunit,*) '> time step number ---> ',numero_step 
...
!***********************************************************************
  ...
  ! mass they get from each progenitor
    ...
    integer(kind=4)    :: i,j,indexp
    type(halo),pointer :: h
    type(tree_struct), pointer :: htr
    real(kind=4)       :: ft,final_mass
    ...
    do i = 1,nb_of_halos(numero_step)  + nb_of_subhalos(numero_step)
       ...
       do j = 1,htr%my_fathers%nb_fathers
          htr%my_fathers%mass_fathers(j) = htr%my_fathers%mass_fathers(j) / h%m*100.0
       end do
...
#ifndef BIG_RUN
          htr%my_fathers%mass_fathers(i) = htr%my_fathers%mass_fathers(i) + mass(indexp)
#else
          htr%my_fathers%mass_fathers(i) = htr%my_fathers%mass_fathers(i) + massp 
#endif
          return
       endif
    end do
...
#ifndef BIG_RUN
       htr%my_fathers%mass_fathers(new_fathers_nb) = mass(indexp)
#else
       htr%my_fathers%mass_fathers(new_fathers_nb) = massp
#endif
