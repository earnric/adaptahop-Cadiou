...
!=======================================================================
  ...
  call change_pos
  if (megaverbose) write(errunit,*) '> Done with change_pos'
  ! action neighbors
  call create_tree_structure
  if (megaverbose) write(errunit,*) '> Done with create_tree_structure'
  call compute_mean_density_and_np
  if (megaverbose) write(errunit,*) '> Done with compute_mean_density_and_np'
  ! action adaptahop
  call find_local_maxima
  if (megaverbose) write(errunit,*) '> Done with find_local_maxima'
  call create_group_tree
  if (megaverbose) write(errunit,*) '> Done with create_group_tree'
  call change_pos_back
  if (megaverbose) write(errunit,*) '> Done with change_pos_back'
  ! check that we have halos
  call count_halos
  if (megaverbose) write(errunit,*) '> Done with count_halos'
  if(method.eq."BHM") then
     if(numero_step.eq.1) then
        open(unit=456,form='formatted',status='unknown',file='nb_extra_sub_BHM.tmp')
     end if
  end if
...
!=======================================================================
  ...
  implicit none
  integer(kind=4) :: ihalo,inode,isub
  integer(i8b)    :: ip
...
!  real(kind=8)    :: masstest
  ...
  if(ex_nb_of_structs .le.0) then
     ...
     call select_with_MS_method
     if(.not.allocated(ex_liste_parts)) allocate(ex_liste_parts(nbodies))
     ex_liste_parts   = liste_parts
     ex_nb_of_structs = nstruct
     allocate(ex_level(ex_nb_of_structs))
  ...
  if(.not.allocated(ex_liste_parts)) stop '> ex_liste_parts not allocated'
  if(.not.allocated(ex_level))       stop '> ex_level not allocated'
  ...
  ! output info nb of subhalos added compared to the expected number from either DPM or MSM methods

  write(456,*) numstep,nb_of_halos,nb_of_subhalos,n_extrasub
...
!=======================================================================
  ...
  integer(i8b), dimension(nvoisins):: iparnei
  real(kind=8)                        :: densav
...
!=======================================================================
  ...
  integer(i8b)             :: ipar,idist,iparid,iparsel,igroup,nmembmax,nmembtot
  integer(i8b),allocatable :: nmemb(:)
...
!$OMP PARALLEL DO &
  !$OMP DEFAULT(SHARED) &
  !$OMP PRIVATE(igroup,rhot,posg,masstmp,igroupref,posref,rsquare,densmoy,truemasstmp)
  do igroup=1,ngroups
...
!$OMP END PARALLEL DO
  ...
  enddo
  if (verbose) write(errunit,*) 'treat_particles done, now to create_nodes'
...
!!$        call do_colorize(icolor_select,igroup,igr,rhot)
        ...
        call do_colorize_ownstack(igroup,igr,rhot) !YDdebug
     endif
  enddo
...
!=======================================================================
  ...
  integer(i8b) :: ipar, iparold
  integer(kind=4) :: igroup,igroupref, imass
  logical         :: first_good
...
!=======================================================================
subroutine do_colorize_ownstack(igroup, igr, rhot)
  use stack
  implicit none
  integer(kind=4) :: igroup,igr,old_col,igroup_tmp
  integer(kind=4) :: ineig,igroup2,igr2,neig
  real(kind=8)  :: rhot
  type(stack_var) :: s

  old_col = color(igr)
  if (old_col .eq. icolor_select) return
  ...
  do while (.not. empty(s))
     ...
     neig=group(igroup_tmp)%nhnei
     do ineig=1,group(igroup_tmp)%nhnei
        if (group(igroup_tmp)%rho_saddle_gr(ineig).gt.rhot) then
           ! We connect this group to its neighbourg
           igroup2=group(igroup_tmp)%isad_gr(ineig)
           igr2=igroupid(igroup2)
           if (color(igr2).eq.0) then
              color(igr2) = icolor_select
              call push(s, igroup2)
           elseif (color(igr2).ne.icolor_select) then
              write(errunit,*) 'ERROR in do_colorize_ownstack : color(igr2) <> icolor_select'
     ...
     enddo
     group(igroup_tmp)%nhnei=neig
  end do



end subroutine do_colorize_ownstack
!=======================================================================
...
!=======================================================================
  ...
  implicit none
  integer(i8b) :: ipar1,ipar2
  integer(kind=4) ::igroup2,ineig,idist,igroup1,ineig2
...
!=======================================================================
  ...
  integer(i8b) :: ipar,idist
  integer(kind=4) :: icell_identity,inccellpart
  real(kind=8)    :: dist2(0:nvoisins)
  integer(i8b) :: iparnei(nvoisins)
...
!=======================================================================
  ...
  integer(kind=4) :: icellidin,icell_identity,inccellpart,ic
  integer(i8b) :: iparid, iparcell, idist
  ...
  integer(i8b) :: i,npart_pos_this_node
  ...
  do while (icell_identity.ne.0)
     ...
     dx=dabs(pos_cell(1,icell_identity)-poshere(1))
     ! dx=max(0.,min(dx,real(xlong,8)-dx)-sc)
     dx=max(0.,min(dx,xlong-dx)-sc)
     dy=dabs(pos_cell(2,icell_identity)-poshere(2))
     dy=max(0.,min(dy,ylong-dy)-sc)
     dz=dabs(pos_cell(3,icell_identity)-poshere(3))
     dz=max(0.,min(dz,zlong-dz)-sc)
     ...
     if (firstchild(icellid_out) < 0) then
        if (discell2(ic).lt.dist2(nvoisins)) then
           ...
           do i=npart_pos_this_node+1,npart_pos_this_node+mass_cell(icellid_out)
              iparcell=idpart(i)
              dx=dabs(pos(iparcell,1)-poshere(1))
              !dx=max(0.,min(dx,real(xlong,8)-dx))
              dx=max(0.,min(dx,xlong-dx))
              dy=dabs(pos(iparcell,2)-poshere(2))
              !dy=max(0.,min(dy,real(ylong,8)-dy))
              dy=max(0.,min(dy,ylong-dy))
              dz=dabs(pos(iparcell,3)-poshere(3))
              !dz=max(0.,min(dz,real(zlong,8)-dz))
              dz=max(0.,min(dz,zlong-dz))
...
!=======================================================================
  ...
  integer(kind=i8b) :: inccell,idmother,ipar
  integer(kind=i8b) :: npart_this_node,npart_pos_this_node
  integer(kind=i8b) :: ncell
...
!=======================================================================
   ...
   integer(kind=4)           :: nlevel
   integer(i8b)         :: npart_pos_this_node,npart_this_node
   real(kind=8)              :: pos_ref(3,0:7)
   real(kind=8)              :: pos_this_node(3)   
   integer(i8b)         :: ipar,icid,inccell
   integer(kind=4)           :: j,nlevel_out
   integer(i8b), external :: icellid
   integer(i8b)           :: npart_pos_this_node_out,npart_this_node_out
   integer(i8b)           :: incsubcell(0:7),nsubcell(0:7)
   real(kind=8)              :: xtest(3),pos_this_node_out(3)
   integer(i8b)           :: idmother,idmother_out

   integer(i8b)           :: ncellmx_old
   integer(i8b), allocatable      :: mass_cell_tmp(:),sister_tmp(:),firstchild_tmp(:)
...
!=======================================================================
  ! What is this routine about? It is not used at all...
  implicit none
  real(kind=8), parameter :: accurac=1.e-6

  integer(kind=4), allocatable, dimension(:) :: idgene
  ...
  integer(i8b) :: i,niter,j,ipar,jpar,idd,jdd,incdege
  integer(i8b) :: idum
