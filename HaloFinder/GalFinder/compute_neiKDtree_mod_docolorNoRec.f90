...
!=======================================================================
  ...
  real(kind=8)                 :: rhot
  integer(kind=4)              :: icolor_select,igroup,icolor,igr,igr_eff
...
!=======================================================================
subroutine do_colorize(icolor_select,igroup,igr,rhot)
!=======================================================================
! recursive subroutine do_colorize open with a do while and allocates
!=======================================================================
  implicit none
  integer(kind=4) :: icolor_select,igroup,igr
  integer(kind=4) :: ineig,igroup2,igr2,neig
  real(kind=8)  :: rhot

  integer(kind=4), allocatable, dimension(:) :: igroupstore,igrstore, &
       &                         igroupstorenew,igrstorenew
  integer(kind=4) :: ngrh,ngrhnew,ngrhnewmax,i,igroup2h,igr2h
  logical :: done

  allocate(igroupstore(1),igrstore(1))
  ngrh=1
  igroupstore(1)=igroup
  igrstore(1)=igr
  done=.false.
  ...
  do while (.not. done)
     ngrhnewmax=0
     do i=1,ngrh
        igroup2h=igroupstore(i)
        ngrhnewmax=ngrhnewmax+group(igroup2h)%nhnei
     enddo
     allocate(igroupstorenew(ngrhnewmax),igrstorenew(ngrhnewmax))
     ngrhnew=0
     do i=1,ngrh
        igroup2h=igroupstore(i)
        igr2h=igrstore(i)
        color(igr2h)=icolor_select
        neig=group(igroup2h)%nhnei
        do ineig=1,group(igroup2h)%nhnei
           if (group(igroup2h)%rho_saddle_gr(ineig).gt.rhot) then
! We connect this group to its neighbourg
              igroup2=group(igroup2h)%isad_gr(ineig)
              igr2=igroupid(igroup2)
              if (color(igr2).eq.0) then
                 ngrhnew=ngrhnew+1
                 igroupstorenew(ngrhnew)=igroup2
                 igrstorenew(ngrhnew)=igr2
...
! are ranked in decreasing order)
        ...
        enddo
        group(igroup2h)%nhnei=neig        
     enddo
     write(*,*)'deallocate(igroupstore,igrstore)',ngrh
     deallocate(igroupstore,igrstore)
     if (ngrhnew > 0) then
        ngrh=ngrhnew
        allocate(igroupstore(ngrh))
        allocate(igrstore(ngrh))
        igroupstore(1:ngrh)=igroupstorenew(1:ngrh)
        igrstore(1:ngrh)=igrstorenew(1:ngrh)
     else
        done=.true.
     endif
     write(*,*)'deallocate(igroupstorenew,igrstorenew)',ngrhnewmax
     deallocate(igroupstorenew,igrstorenew)
  enddo
